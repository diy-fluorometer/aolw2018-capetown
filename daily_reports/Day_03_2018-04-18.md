...
The circiut i had in mind for that can be seen in figure ()
However, the transistor did not seem to switch with the voltage supplied by the audio output. Anothe rpossibility would be an op-Amp...
to make it work i devised this circuit in figure 2. It basically supplies the LED with a current of roughly 3 V and on top of that the voltage supplied by the audio output. So the resulting voltage 
